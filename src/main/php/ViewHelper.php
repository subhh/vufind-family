...
{
    public function __invoke (SolrDefault $record) : ?string
    {
        if ($record->getHierarchyTopID()[0] === $record->getUniqueID()) {
            if ($view = $this->getView()) {
                return $view->render('subhh-vufind-family/family.phtml', ['record' => $record]);
            }
        }
        return null;
    }
